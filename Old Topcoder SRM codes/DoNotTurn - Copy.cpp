...
int minimumTurns(int N, int _X0, int _A, int _B, int _Y0, int _C, int _D, int _P, int M)
{
   while(!Q.empty())
    ...
    for(int i=0;i<4;i++)
    {
         vector<int> curr; curr.pb(0);curr.pb(0); curr.pb(i);curr.pb(0);
         Q.push_back(curr);
    }
    ...
    {
        vector<int> curr=Q.front(); Q.pop_front();

        int x=curr[0],y=curr[1],dir=curr[2],cd=curr[3];
//
        if( (x==N-1)&&(y==N-1) )
             {
                 return cd;
        ...
        //cout<<" *****  "<<x<<"  "<<y<<"  "<<dir<< "  "<<cd<<endl;
           ...
           for(int i=0;i<4;i++)
           {
               ...
               //cout<<nx<<"  "<<ny<<"  "<<i<<"  "<<cd+add<<endl;
                 ...
                 vector<int> tp; tp.pb(nx); tp.pb(ny); tp.pb(i); tp.pb( new_d);
