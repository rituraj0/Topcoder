...
int minimalTime(vector <int> _wt, vector <int> _time, vector <string> _trust , int _bs)
{
   ...
   dist[ incode( (1<<n)-1 ,1) ]=0; // second filed 1 emans  map cave k  entrance pe hai ,0 means  on exit pe
   Q.insert( mp(0, incode( (1<<n)-1 ,1)  ) );
   ...
   while( Q.size() > 0)
   {
       ...
       if( (node.X==0)&& (node.Y==0))
        ...
        int om=node.X , mpos=node.Y;

        int nmp=1-mpos;

        for(int i=om; i>0; i=(i-1)&om )//moving this susbset
        {
            if(i==om) continue;
